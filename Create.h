...
int LocateVex(VGraph G, VexType v) {//确定点v在G中的位置
...
void ShowMatrix(VGraph G) {//显示对称矩阵
...
int CreateMatrix(VGraph& G) {//用随机数填充一个对称矩阵
...
int CreateVillage(VGraph& G) {//采用邻接矩阵表示法，创建有向网G
	int i, j;
...
void ShortPath(VGraph G) {//用Floyd算法求有向网G中各对顶点i和j之间的最短路径 
	int i, j, k;
...
void DisplayPath(VGraph G, int begin, int end) {//显示最短路径
...
void SumDestination(VGraph G) {	//计算并保存各顶点的偏心度到顺序表
	//初始化偏心度矩阵
...
void LocateHospital(VGraph G, int& Hospital) {//找到中心点
	Hospital = 0;//记录选址下标
...
void ShowDestination(VGraph G, int Hos) {//显示路径和长度
